...
def rcd(): 
    ...
    for loc in range(1, 6):
        connection = cx_Oracle.connect("scott","tiger","192.168.0.69:1521/orcl")
        cur = connection.cursor()
        sql = "SELECT TNWH_NO, TNWH_FORECAST from SM_TNWH_TB WHERE TNWH_DATE = :txt1 AND TNWH_TYPE = :txt2 AND TNWH_LOC = :txt3 AND TNWH_TIME = :txt4"
        cur.execute(sql,[today, 3, loc, str(time.localtime().tm_hour)])  #txt = 시간 앞 , txt2 = 시간 뒤 , txt3 = 날짜 //// txt=18, txt2=24 ,
        forecast = []
        weathrtNo = []
        ...
        # weatherList = ['구름 많음','흐리고 비','맑음','흐림','비','구름조금']
        #검색값을 설정해야함.
        ...
        for wh in weathrtNo:
            ...
            for i in range(len(forecast)):
                w = forecast[i]
                if w in ("천둥번개", "비", "구름많고 비", "흐리고 비", "빗방울", "소나기", "가끔 비", "한때 비", "가끔 눈", "한때 눈", "가끔 비 또는 눈", "한때 비 또는 눈", "가끔 눈 또는 비", "한때 눈 또는 비"):
                    r = '비오는' 
                elif w in ('구름 많음', '구름많음', '흐림', '구름조금'):
                    r = '흐린'
                elif w in ('맑음'):
                    # r = '날씨 좋은'
                    r = '맑은'
            ...
            # 추천요리
            r_results = YoutubeSearch(r+" 날 해먹을 만한 음식", max_results=10).to_dict() #
            ...
            for i in r_results:
                ...
                cur.execute(sql,[r_title, r_video, wh])
                connection.commit()
                connection.close()
            ...
            for i in r_results:
                ...
                cur.execute(sql,[r_title, r_music, wh])
                connection.commit()
                connection.close()


            # 추천여행지
            r_plusurl = urllib.parse.quote_plus(r+" 날 여행")
            ...
            for a in r_As:
                ...
                cur.execute(sql,[r_title,r_href,wh])
